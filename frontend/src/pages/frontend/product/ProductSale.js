...
const ProductSale = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const result = await productService.productsale(limit);
              ...
              <div key={product.id} className="col-md-4 col-lg-3">
                <div className="product-card position-relative">
                  ...
                  <div className="card-detail text-center pt-3 pb-2">
                    ...
                    <div>
                      <span className="item-price original-price text-muted fs-3 fw-light" style={{ textDecoration: 'line-through' }}>
                        {product.price}$
                      </span>

                    </div>
                    <span className="item-price text-primary fs-3 fw-light"> sale {product.sale.pricesale}$</span>
                  </div>
                </div>
              </div>
            )
